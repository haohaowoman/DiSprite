...
BOOL DiControlPropertyPack::OnGetPropertyValue(const PropProxyInfo& propProxy, _variant_t & val)
{
    ...
    //if (bSucc)
    //{
    //    ASSERT(propProxy.ValueType == val.vt);
    //}
...
BOOL BasicCtrlPropertyPack::OnSetPropertyValue(const PropProxyInfo& propProxy, const _variant_t& val)
{
	...
	if (!bsuc)
	{
		...
		case SCREEN_INDEX_PROXY:
			...
			break;
		case TOP_PROXY:
			Control()->Top(static_cast<int>(val));
			break;
		case LEFT_PROXY:
			Control()->Left(static_cast<int>(val));
			break;
		case BOTTOM_PROXY:
			Control()->Bottom(static_cast<int>(val));
			break;
		case RIGHT_PROXY:
			Control()->Right(static_cast<int>(val));
			break;
		case TOGGEL_PAGEINDEX_PROXY:
...
void BasicCtrlPropertyPack::ProducePropertiesMap(vector<PropProxyInfo>& propMap)
{
	...
	PropProxyInfo tempProps[] = 
	{
		...
		{_T("Bottom"), BOTTOM_PROXY, VT_INT, _T("右下角Y坐标"), TRUE},
		{_T("Right"), RIGHT_PROXY, VT_INT, _T("右下角X坐标"), TRUE},
		...
		{_T("控件类型"), CONTROL_TYPE_PROXY, VT_INT, _T("控件类型"), FALSE},
	};
...
void TextInputPack::ProducePropertiesMap(vector<PropProxyInfo>& propMap)
{
	...
	PropProxyInfo tempProps[] = 
	{
		{_T("录入模式"), INPUT_MODE_PROXY, VT_UI2, _T("0x00：从新录入 0x01：修改文本"), TRUE},
