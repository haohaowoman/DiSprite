			...
			if (SUCCEEDED(hr))
			{
				...
				if (SUCCEEDED(hr = pageList != NULL ? S_OK : E_FAIL))
				{
					...
					for (long i = 0; SUCCEEDED(hr) && i < count && NULL != (page = pageList->item[i]); i++)
					{
						...
						dIPage->ImagePath(NULL != at ? at->nodeValue : _T(""));
...
BOOL DiDocument::DiProject::SaveFile()
{
	...
	if (SUCCEEDED(hr))
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			if (NULL != dIPage)
			{
				...
				page->setAttribute(_bstr_t(ImagePath_Node), (_variant_t)dIPage->ImagePath());
	...
	return SUCCEEDED(hr) && SaveProjectFile(_T(".\\"));
}
