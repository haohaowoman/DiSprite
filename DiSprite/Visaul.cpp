...
void DiControl::ImageVisaul::SetImageUri(const CString& uri)
{
    if (uri != m_imagePath)
    {
        ...
        {
            m_image = Image::FromFile(uri.AllocSysString());
        }
        m_imagePath = uri;
    }
}
...
Size DiControl::ImageVisaul::GetImageSize() const
{
    if (nullptr != m_image)
    {
		RectF bounds;
		Unit ut = UnitPixel;
		m_image->GetBounds(&bounds, &ut);
        return Size(/*m_image->GetWidth(), m_image->GetHeight()*/(int)bounds.Width, (int)bounds.Height);
    }
    return Size();
}
...
void DiControl::ImageVisaul::OnRender(Graphics& dc)
{
    if (nullptr != m_image)
    {
		...
		at.SetWrapMode(WrapModeTileFlipXY);
		dc.SetInterpolationMode(InterpolationModeNearestNeighbor);
        dc.DrawImage(m_image, GetLayout(), 
			0, 0, 
			/*GetLayout().Width*/m_image->GetWidth(), 
			/*GetLayout().Height*/m_image->GetHeight(), 
			Gdiplus::UnitPixel
			);
		Gdiplus::Pen borderPen(Gdiplus::Color::DarkGray);
		
		dc.DrawRectangle(&borderPen, GetLayout());
    }
}
