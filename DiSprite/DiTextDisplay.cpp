...
CString DiControl::DiRTCText::RTCFormat() const
{
	CString format;
	try
	{
		CA2T tStr((LPCSTR)(Second.Page));
		format = tStr;
	}
	catch(...)
	{
		TRACE("Ascii to unicode error!");
	}
	return format;
}

void DiControl::DiRTCText::RTCFormat(const CString& val)
{
	ZeroMemory(Second.Page, sizeof(PageData));

	int count = val.GetLength();

	for (int i = 0; i < count && i < sizeof(PageData); i++)
	{
		Second.Page[i] = (BYTE)(val.GetAt(i) & 0xFF);
	}
}
